  ...
  try {
    ...
    const result = await withTransaction(async (client) => {
      const subscriptionResult = await client.query<{
        id: string; code: string; status: string; pause_limit: number; pauses_used: number;
        pickup_point_name: string | null; full_name: string; phone: string | null;
      }>(
      ...
      const lastDateResult = await client.query<{ last_date: string }>(
        `SELECT MAX(service_date)::text AS last_date FROM subscription_days WHERE subscription_id = $1`,
        [id]
      );
      ...
      await client.query(
        ...
        [id, JSON.stringify({ serviceDate, replacementDate, fullName: subscription.full_name, phone: subscription.phone, pickupPointName: subscription.pickup_point_name })]
      );
