...
export default function SubscriptionCalendar() {
  ...
  function addNextDay() {
    if (selected.length >= 30 || nextIndex >= days.length) return;
  ...
  const nextDayForButton = selected.length < 30 ? days[nextIndex] : null;
  ...
  return (
    <section id="subscription" className="subscription-section">
      ...
      <div className="calendar-actions consecutive-actions">
        ...
        <span className="calendar-action-note">Можно выбрать до 30 дней подряд.</span>
      </div>

      <div className="calendar-scroll" ref={scrollRef} onScroll={syncCalendarSlider}>
        {days.map((item, index) => {
          const isSelected = selectedSet.has(item.id);
          const isNext = index === nextIndex && selected.length < 30;
          ...
          return (
            <article id={`meal-day-${item.id}`} key={item.id} className={`meal-day ${isSelected ? "selected" : ""} ${isNext ? "next-available" : ""}`}>
              <button type="button" className="meal-day-preview" onClick={() => setDetailsIndex(index)} aria-label={`Открыть меню на ${formatLongDate(item)}`}>
                ...
                <span className="meal-details-hint">Нажмите, чтобы посмотреть состав и КБЖУ</span>
              </button>
              <button type="button" className={`add-next-day-button ${isSelected ? "is-selected" : ""}`} disabled={!isNext} onClick={addNextDay}>
                {isSelected ? "День выбран" : index === 0 && !selected.length ? "Выбрать первый день" : "Добавить следующий день"}
              </button>
            </article>
          );
        })}
            ...
            <div className="meal-details-modal">
              ...
              <div className="meal-details-footer">
                ...
                <button type="button" disabled={!nextDayForButton} onClick={addNextDayFromModal}>
                  {nextDayForButton ? `Выбрать следующий день — ${nextDayForButton.day} ${nextDayForButton.monthLabel}` : "Выбрано 30 дней"}
                </button>
              </div>
            </div>
          </div>
